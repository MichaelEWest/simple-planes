package xyz.roqadaq.simpleplanes.upgrades.floating;
...
public class FloatingUpgrade extends Upgrade {
    ...
    public void render(PoseStack matrixStack, SubmitNodeCollector snc, int packedLight, PlaneRenderState state) {
        super.render(matrixStack, snc, packedLight, state);

        if (planeEntity.getType() == SimplePlanesEntities.CARGO_PLANE.get()) {
            snc.submitModel(UpgradesModels.WOODEN_CARGO_FLOATING, state, matrixStack, state.materialTexture, packedLight, OverlayTexture.NO_OVERLAY, state.outlineColor, null);
}
}
    @Override
