...
public class CargoPlaneEntity extends PlaneEntity {
    ...
    public void readNewCargoUpgradePacket(Identifier upgradeID, ByteBuf packetBuffer) {
    ...
    public void dropPayload() {
        for (LargeUpgrade upgrade : largeUpgrades) {
            if (upgrade.canBeDroppedAsPayload()) {
                ...
                if (level().isClientSide()) {
                    ClientPacketDistributor.sendToServer(new DropPayloadPacket());
}
                break;
}
}
    ...
    protected void readAdditionalSaveData(ValueInput in) {
        ...
        largeUpgrades.clear();
        in.childrenListOrEmpty("large_upgrades").forEach(entry -> {
            String id = entry.getStringOr("id", "");
            UpgradeType upgradeType = SimplePlanesRegistries.UPGRADE_TYPE.getOptional(Identifier.parse(id)).orElse(null);
            if (upgradeType != null) {
                Upgrade upgrade = upgradeType.instanceSupplier.apply(this);
                if (upgrade instanceof LargeUpgrade largeUpgrade) {
                    entry.read("nbt", CompoundTag.CODEC).ifPresent(largeUpgrade::deserializeNBT);
                    largeUpgrades.add(largeUpgrade);
                }
            }
        });
    ...
    protected void addAdditionalSaveData(ValueOutput out) {
        ...
        ValueOutput.ValueOutputList listOut = out.childrenList("large_upgrades");
        for (LargeUpgrade upgrade : largeUpgrades) {
            ValueOutput entry = listOut.addChild();
            entry.putString("id", SimplePlanesRegistries.UPGRADE_TYPE.getKey(upgrade.getType()).toString());
            entry.store("nbt", CompoundTag.CODEC, (CompoundTag) upgrade.serializeNBT());
        }
    }
    @Override
